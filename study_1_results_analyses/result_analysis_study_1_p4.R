...
# calculate potential scale reduction factor, or PSRF, i.e., R hat.
...
# mean(test_sum$Rhat, na.rm = T)
# median(test_sum$Rhat, na.rm = T)
# range(test_sum$Rhat, na.rm = T)

rm(chains_all, temp_log)
# save all results as a image
save.image(file = paste0('../results_new/results_rhat/Mixture_', condition_idx, "_results_Rhat.RData"))
