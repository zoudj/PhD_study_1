...
# not all functions are used in the analysis
...
# retrieve class proportion info
...
# retrieve ability estimates (using EAP)
...
# set up a data frame for use in drawing density plots
...
# retrieve class membership crosstabulation
...
draw_density_line <- function(data = NULL, save_as = NULL){
  ...
  # plotting
  p_density <- ggplot(data = plot_df, aes(x = ability, group = gc, color = gc)) + 
    geom_density(aes(linetype = group)) + 
    scale_linetype_manual(values = c("true" = "solid", "est" = "dashed")) + geom_jitter(data = plot_df_mc, aes(x = ability, y = 0, color = mc), size = 0.05, height = 0.01, alpha = 0.5) + 
    labs(x = 'Ability', y = 'Density', color = "Types") + 
    ggtitle("Separate Density Plot of Estimated and True Abilities") + 
...
# drawing scatter plots showing individuals that are misclassified 
...
# drawing density plots without classification
